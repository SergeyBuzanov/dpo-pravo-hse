...
test('карточка формата: номер ступени, водяной знак и строка фактов', () => {
  ...
  list.forEach((card, i) => {
    ...
    // Пометка с названием документа СНЯТА с обложки (владелец 05.09.2026,
    // вечером того же дня: надпись закрывает сам бланк). Название документа
    // осталось там, где не мешает картинке: в имени действия у кнопки и
    // строкой заказчика «Итоговый документ: …» в теле карточки.
    assert.doesNotMatch(card, /dpo-format-chip/, 'пометка вернулась на обложку бланка');
