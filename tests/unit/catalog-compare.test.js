...
// Сравнивать программы по числу модулей нельзя: «18» и «6» ничего не говорят
// о содержании (владелец 05.09.2026, вечер). В карточке теперь лежат САМИ
// названия, а число таблица считает из того же списка – второго источника
// одних и тех же данных на странице по-прежнему нет.
test('в карточке лежат названия модулей, а не только их число', () => {
  const lists = CARDS.map((tag) => (tag.match(/data-cmp-modules="([^"]*)"/) || [])[1] || '');
  assert.ok(lists.every(Boolean), 'у карточки нет списка модулей');
  assert.ok(
    lists.every((v) => /[А-Яа-яЁёA-Za-z]/.test(v)),
    'в data-cmp-modules только число – сравнивать нечего',
  );
  // Разделитель тот же, что у строки «Для кого»: скрипт разбирает по нему.
  const counted = lists.map((v) => v.split(' · ').filter(Boolean).length);
  assert.ok(Math.min(...counted) >= 3, `самый короткий список модулей – ${Math.min(...counted)}`);
});

test('таблица показывает модули списком, а число считает из того же списка', () => {
  assert.match(SCRIPT, /\['Модули',[^\]]*'list'\]/, 'строка «Модули» не списочная');
  assert.match(SCRIPT, /'Модулей'/, 'строка с числом модулей пропала');
  assert.doesNotMatch(SCRIPT, /data-cmp-module[s]?-(count|list)/, 'число или список приехали вторым атрибутом');
});
