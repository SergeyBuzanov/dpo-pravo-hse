 ...
 *
 * Отдельно про walk: своего места на экране у него нет и не нужно – это
 * СОСТАВНАЯ ЧАСТЬ двух других поз. Шаг рисует и `walkAcross` (выход в
 * содержимое каталога и «Топ-5»), и вторая половина `leave` (уход после
 * взмаха). Числился «простаивающим» по недосмотру: играет он при каждом
 * выходе вороны и при каждом открытии окна поддержки.
 *
...
test('walk – общий шаг двух поз, а не простаивающая анимация', () => {
  // Если шаг перестанут звать из walkAcross или leave, ворона поедет по
  // экрану без движения ног – в браузере это видно, а по коду нет.
  assert.match(MASCOT, /A\.walkAcross = \{[\s\S]{0,200}?A\.walk\.fn\(/, 'walkAcross больше не рисует шаг');
  assert.match(MASCOT, /A\.leave = \{[\s\S]{0,400}?A\.walk\.fn\(/, 'leave больше не рисует шаг');
});
