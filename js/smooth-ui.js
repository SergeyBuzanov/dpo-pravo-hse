...
`.trim();
  ...
  const markRevealTargets = () => {
    ...
    // Здесь стояли .dpo-top5-grid и .dpo-why-card – классов с такими
    // именами в разметке нет (замер 05.09.2026: ноль совпадений), и каскад
    // в «Топ-5» и «Почему выбирают» не работал ни дня. Селекторы выправлены
    // по настоящей разметке 07.09.2026 (решение владельца): плитки лежат в
    // .dpo-top5-track, строки преимуществ – в .dpo-why-list. Сторож –
    // tests/unit/reveal-cascade.test.js: каждый класс списка обязан
    // встречаться в разметке лендинга.
    const CARD_CASCADE = [
