/*
 * Крупный образец итогового документа (владелец 05.09.2026).
 *
 * Образец в ступени траектории – кнопка `[data-doc-preview]`; клик или Enter
 * открывает бланк во весь экран. Наведение только подсказывает (увеличение
 * внутри рамки живёт в CSS блока): на телефоне наведения нет вовсе, а с
 * клавиатуры оно недостижимо, поэтому единственный способ открыть образец –
 * то же действие, что работает всюду.
 *
 * Словарь окна – окна сравнения программ и заявки: затемнение, бумага,
 * крестик 40px, Escape, возврат фокуса, фон получает inert. Слушатель –
 * делегированием по документу: рантайм сборщика клонирует разметку при
 * перерисовке, и слушатель, повешенный на саму кнопку, пропал бы.
 */
(function () {
  'use strict';
  ...
  function onKey(e) {
    ...
    if (e.key !== 'Tab') return;
    // В окне ровно одна мишень – крестик, и фокус не должен из неё уходить.
