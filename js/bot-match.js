...
(function (root, factory) {
  ...
  function parseQuery(query) {
    ...
    //
    // Лукахед (?<![а-яa-z]) перед перечислением слов-триггеров – это
    // граница слова без \b (тот же приём, что и в I1: \b не видит границ
    // вокруг кириллицы). Без неё «за» находился ПОДСТРОКОЙ внутри «виза»
    // («ви-за»), и consumeWord ниже стирал «виза» целиком как ложную цену.
    ...
    if (price) {
      ...
      if (!isDuration && !tooSmall) {
        ...
        // price[0] может кончаться пробелом: (\d[\d\s]*) жаден к пробелам
        // и, если денежной единицы нет, «доедает» пробел ПЕРЕД следующим
        // словом (группа тоже необязательная \s* после числа – тот же
        // эффект). Без обрезки этого хвоста consumeWord получал границу
        // совпадения уже внутри следующего слова («до 100000 банкротство»
        // – граница вставала на «б») и стирал его целиком вместе с ценой.
    ...
    out.stems = remaining
      ...
      // не должны сами по себе давать очков – см. STOP_WORDS выше. Сверка –
      // тем же sameStem, каким hits() сравнивает запрос с программой (не
      // точное равенство): иначе словоформа, которую hits() уже считает
      // тем же словом, что стоп-слово, проходит фильтр как содержательная
      // и позже сама попадает в ложное совпадение.
      .filter(function (s) {
