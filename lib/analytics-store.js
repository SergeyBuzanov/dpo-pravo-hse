...
const DIR = process.env.ANALYTICS_DIR
...
function maxBytes() {
  const n = Number(process.env.ANALYTICS_MAX_BYTES);
  return Number.isFinite(n) && n > 0 ? n : 64 * 1024 * 1024;
}

async function eventsDirBytes() {
  let total = 0;
  let names;
  try {
    names = await fsp.readdir(EVENTS_DIR);
  } catch {
    return 0;
  }
  for (const name of names) {
    try {
      const st = await fsp.stat(path.join(EVENTS_DIR, name));
...
async function appendEvents(events) {
  await ensureDirs();
  const extra = events.reduce((n, e) => n + Buffer.byteLength(JSON.stringify(e)) + 1, 0);
  const used = await eventsDirBytes();
  if (used + extra > maxBytes()) {
    console.warn('analytics quota exceeded, drop');
    return;
  }
...
async function loadEventsSince(days) {
  ...
  // Дневные файлы независимы — читаем параллельно, а не по одному;
  // порядок дней сохраняет Promise.all.
  const wantedDays = files
    .map((name) => name.replace('.jsonl', ''))
    .filter((day) => day >= cutoffDay);
  const perDay = await Promise.all(wantedDays.map((day) => readDayEvents(day)));
  const events = [];
