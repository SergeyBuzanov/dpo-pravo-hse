...
const { moscowMinutesOfDay, moscowParts } = require('./moscow-time');
...
const SCHEDULE_FILE = process.env.CATALOG_SCHEDULE_FILE
  ? path.resolve(process.env.CATALOG_SCHEDULE_FILE)
  : dataFile('.catalog-schedule.json');
...
function normalizeProgram(raw, { generateId = true } = {}) {
  ...
  return {
    ...
    // Разведка страниц маркетплейса 09.09.2026: часы, язык и график занятий
    // из блока «Формат обучения», условия оплаты, документы для приёма,
    // преимущества и приложенные файлы. Без своего нормализатора любое
    // новое поле стирается здесь молча при первой же записи каталога.
    hours: plainText(raw.hours, MAX_HOURS),
    language: plainText(raw.language, MAX_TYPE),
    schedule: plainText(raw.schedule, MAX_LIST_ITEM),
    taxRefund: plainText(raw.taxRefund, MAX_HOURS),
    discounts: normalizeList(raw.discounts),
    admissionDocs: normalizeList(raw.admissionDocs),
    advantages: normalizeList(raw.advantages),
...
/**
 * Файлы программы: учебный план и расписание, скачанные к себе
 * scripts/fetch-program-media.js.
 *
 * Правило то же, что у обложек: показываем ТОЛЬКО свой файл. Путь обязан
 * лежать в files/ и состоять из безопасных символов – иначе строка из
 * чужого источника задала бы адрес вида «../../» на странице программы.
 * Ссылка на оригинал хранится рядом: если наш файл потеряется, будет
 * видно, откуда его брали.
 */
const PROGRAM_FILE_RE = /^files\/[a-z0-9_-]+\.pdf$/i;
const FILE_KINDS = new Set(['plan', 'schedule']);
const MAX_FILES = 4;
...
function normalizeFiles(value) {
  ...
  for (const f of value) {
    ...
    const kind = FILE_KINDS.has(f.kind) ? f.kind : null;
    const path = typeof f.path === 'string' && PROGRAM_FILE_RE.test(f.path.trim()) ? f.path.trim() : null;
    if (!kind || !path) continue;
    let url = null;
    try { url = safeHseUrl(f.url); } catch { url = null; }
    if (!url) continue;
    if (out.some((x) => x.kind === kind)) continue;
    out.push({ kind, title: plainText(f.title, MAX_NAME), size: plainText(f.size, MAX_HOURS), url, path });
...
/**
 * Блок «Важно» со страницы программы: срочное объявление центра с датой.
 *
 * Ссылка «Просмотреть» ведёт на ЧУЖОЙ домен – записи вебинаров лежат на
 * сторонних площадках. Требовать hse.ru здесь нельзя, иначе объявление
 * теряет смысл, поэтому проверка другая: только https и только разбираемый
 * адрес. Хост показывается человеку на странице.
 */
const MAX_NOTICE = 400;

function normalizeNotice(value) {
  if (!value || typeof value !== 'object') return null;
  const text = plainText(value.text, MAX_NOTICE);
  if (!text) return null;
  let url = null;
  if (typeof value.url === 'string') {
    try {
      const u = new URL(value.url.trim());
      if (u.protocol === 'https:') url = u.toString();
    } catch {
      url = null;
    }
  }
  return { date: plainText(value.date, MAX_HOURS), text, url };
}

/**
 * Объявления старше этого срока не показываем: блок с надписью «Важно» и
 * датой полугодовой давности читается как заброшенный сайт. Порог – 90
 * календарных дней по Москве. Без даты объявление считается бессрочным
 * (чек-лист, постоянная акция) и остаётся.
 */
...
function isNoticeFresh(notice, now = Date.now()) {
  if (!notice || typeof notice !== 'object' || !notice.text) return false;
  const parsed = parseNoticeDate(notice.date);
  if (!parsed) return true;
  const today = moscowParts(now);
  if (!today) return true;
  const then = Date.UTC(parsed.year, parsed.month - 1, parsed.day);
  const nowUtc = Date.UTC(today.year, today.month - 1, today.day);
  const age = Math.round((nowUtc - then) / 86400000);
  if (age < 0) return true;
  return age <= NOTICE_MAX_AGE_DAYS;
}

/**
 * «Вопросы и ответы» со страницы программы. Ответ бывает длинным (порядок
 * зачисления, срок доступа к материалам), поэтому потолок как у отзыва, а
 * не как у пункта списка.
 */
const MAX_FAQ = 12;
...
function normalizeFaq(value) {
  ...
  for (const item of value) {
    ...
    const q = plainText(item.q, MAX_LIST_ITEM);
    const a = plainText(item.a, MAX_FEEDBACK_TEXT);
    if (!q || !a) continue;
    out.push({ q, a });
...
function normalizeModules(value, max = MAX_MODULES) {
  ...
  const items = value
    .map((m) => {
      ...
      return {
        title: plainText(m.title, MAX_LIST_ITEM),
        hours: plainText(m.hours, MAX_HOURS),
        // Подтемы модуля со страницы маркетплейса. Их бывает до двадцати
        // в модуле, поэтому свой потолок, а не общий MAX_LIST_ITEMS.
        topics: normalizeList(m.topics, MAX_MODULE_TOPICS),
      };
    })
    .filter((m) => m && m.title)
    .slice(0, max);
 ...
 * scripts/fetch-program-descriptions.js – тексты, часы, FAQ, «Важно»;
 * scripts/fetch-program-media.js – обложку и PDF. Редактор админки этих
 * полей не знает: любое сохранение, которое их не передало, обязано взять
 * прежние значения из хранилища, иначе одно нажатие «Сохранить в каталог»
 * обнуляет карточки.
 *
 ...
 *
 * Поля сентября 2026 (часы, скидки, «Важно», FAQ, файлы) добавлены сюда же:
 * нормализатор их уже собирает, но без переноса тонкий PUT из админки
 * подставлял null и молча стирал живые карточки.
 */
