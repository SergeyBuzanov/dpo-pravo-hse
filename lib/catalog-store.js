...
async function loadStore() {
  try {
    ...
    if (err.code === 'ENOENT') {
      return { updated: null, source: null, programs: [], teacherPhotos: null };
    }
    throw err;
  }
}

async function saveStore({ programs, source, updated, teacherPhotos }) {
  ...
  if (teacherPhotos === undefined) {
    photos = (await loadStore()).teacherPhotos;
  }
  const payload = {
      ...
      const n = normalizeProgram(
        {
          ...
          // Обложка скачивается scripts/fetch-program-media.js и в выдаче
          // каталога hse.ru отсутствует – без переноса из локальной копии
          // обновление стирало бы её у всех программ.
