...
// писать настоящие заявки в рабочую копию проекта. DPO_DATA_DIR — боевой
// том, отдельно от document root nginx.
...
/** Потолок журнала, чтобы флуд с разных IP не забил диск. По умолчанию 50 МБ. */
function maxBytes() {
  const n = Number(process.env.APPLICATION_MAX_BYTES);
...
async function assertQuota(extra = 0) {
  const used = await dirBytes();
  if (used + extra > maxBytes()) {
    const err = new Error('хранилище заявок переполнено');
    err.code = 'QUOTA';
    throw err;
  }
}
...
function save(application) {
  const run = async () => {
    ...
    await assertQuota(Buffer.byteLength(line));
    await fsp.appendFile(file, line, { encoding: 'utf8', mode: FILE_MODE });
...
/**
 * Уничтожает заявку по требованию заявителя (privacy.html, разд. 3 и 5):
 * строка убирается из файла месяца, статус – из status.json. Месяц
 * переписывается атомарно через временный файл, как и статусы.
 * @returns {Promise<boolean>} нашлась ли заявка
 */
function remove(id) {
  const run = async () => {
    for (const key of await listMonths()) {
      const recs = await readMonth(key);
      const idx = recs.findIndex((r) => r.id === id);
      if (idx === -1) continue;
      recs.splice(idx, 1);
      const file = path.join(DIR, `${key}.jsonl`);
      const tmp = `${file}.${process.pid}.${crypto.randomBytes(4).toString('hex')}.tmp`;
      await fsp.writeFile(tmp, recs.map((r) => `${JSON.stringify(r)}\n`).join(''), { encoding: 'utf8', mode: FILE_MODE });
